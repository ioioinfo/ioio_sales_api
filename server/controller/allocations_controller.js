    ...
    server.route([
        //查询所有渠道
        {
            ...
            handler: function(request, reply) {
                ...
                //查询所有渠道部门
                server.plugins['models'].allocations.get_allocations(info,function(err,rows){
                    if (!err) {
						ep.emit("rows", rows);
					}else {
						ep.emit("rows", []);
				...
				server.plugins['models'].allocations.account_allocations(info,function(err,rows){
                    if (!err) {
        ...
        //新增
        {
            ...
            handler: function(request, reply){
                ...
                server.plugins['models'].allocations.save_allocation(allocation, function(err,result){
        ...
        //删除
        {
            ...
            handler: function(request, reply){
                ...
                server.plugins['models'].allocations.delete_allocation(id, function(err,result){
                    if (result.affectedRows>0) {
                        return reply({"success":true,"service_info":service_info});
                    }else {
                        return reply({"success":false,"message":result.message,"service_info":service_info});
        ...
        //查询渠道
        {
            ...
            handler: function(request, reply) {
                ...
                //查询渠道部门
                server.plugins['models'].allocations.search_allocation_byId(id,function(err,rows){
                    if (!err) {
                        ep.emit("rows", rows);
                    }else {
                        ep.emit("rows", []);
        ...
        //更新渠道
        {
            ...
            handler: function(request, reply){
                ...
                server.plugins['models'].allocations.update_allocation(allocation, function(err,result){
                    if (result.affectedRows>0) {
                        return reply({"success":true,"service_info":service_info});
                    }else {
                        return reply({"success":false,"message":result.message,"service_info":service_info});
