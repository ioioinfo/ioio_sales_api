    ...
    server.route([
        //查询所有渠道
        {
            ...
            handler: function(request, reply) {
                ...
                //查询所有渠道部门
                server.plugins['models'].cpq_details.get_cpq_details(info,function(err,rows){
				...
				server.plugins['models'].cpq_details.account_cpq_details(info,function(err,rows){
                    if (!err) {
                ...
                get_product(detail.product_id,function(err,rows){
					if (!err) {
						if (rows.rows.length>0) {
                            ...
                            server.plugins['models'].cpq_details.save_cpq_detail(detail, function(err,result){
        ...
        //删除
        {
            ...
            handler: function(request, reply){
                ...
                server.plugins['models'].cpq_details.delete_cpq_detail(id, function(err,result){
                    if (result.affectedRows>0) {
                        return reply({"success":true,"service_info":service_info});
                    }else {
                        return reply({"success":false,"message":result.message,"service_info":service_info});
        ...
        //查询渠道
        {
            ...
            handler: function(request, reply) {
                ...
                //查询渠道部门
                server.plugins['models'].cpq_details.search_cpq_detail_by_id(id,function(err,rows){
                    if (!err) {
                        ep.emit("rows", rows);
                    }else {
                        ep.emit("rows", []);
                ...
                get_product(detail.product_id,function(err,rows){
					if (!err) {
						if (rows.rows.length>0) {
                            ...
                            server.plugins['models'].cpq_details.update_cpq_detail(detail, function(err,result){
                                if (result.affectedRows>0) {
                                    return reply({"success":true,"service_info":service_info});
                                }else {
                                    return reply({"success":false,"message":result.message,"service_info":service_info});
