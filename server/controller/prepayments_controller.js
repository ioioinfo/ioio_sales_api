    ...
    server.route([
        //查询所有渠道
        {
            ...
            handler: function(request, reply) {
                ...
                //查询所有渠道部门
                server.plugins['models'].prepayments.get_prepayments(info,function(err,rows){
                    if (!err) {
						ep.emit("rows", rows);
					}else {
						ep.emit("rows", []);
				...
				server.plugins['models'].prepayments.account_prepayments(info,function(err,rows){
                    if (!err) {
        ...
        //新增
        {
            ...
            handler: function(request, reply){
                ...
                server.plugins['models'].prepayments.save_prepayment(prepayment, function(err,result){
        ...
        //删除
        {
            ...
            handler: function(request, reply){
                ...
                server.plugins['models'].prepayments.delete_prepayment(id, function(err,result){
                    if (result.affectedRows>0) {
                        return reply({"success":true,"service_info":service_info});
                    }else {
                        return reply({"success":false,"message":result.message,"service_info":service_info});
        ...
        //查询渠道
        {
            ...
            handler: function(request, reply) {
                ...
                //查询渠道部门
                server.plugins['models'].prepayments.search_prepayment_by_id(id,function(err,rows){
                    if (!err) {
                        ep.emit("rows", rows);
                    }else {
                        ep.emit("rows", []);
        ...
        //更新渠道
        {
            ...
            handler: function(request, reply){
                ...
                server.plugins['models'].prepayments.update_prepayment(prepayment, function(err,result){
                    if (result.affectedRows>0) {
                        return reply({"success":true,"service_info":service_info});
                    }else {
                        return reply({"success":false,"message":result.message,"service_info":service_info});
